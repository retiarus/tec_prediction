...
class SequenceLoader(data.Dataset):
    ...
    def __init__(self, name, path_files, seq_length_min, step_min,
                 window_train, window_predict):
        """Init function."""
        ...
        file = f'samples_{self.name}_{self.seq_length_min}_{self.step_min}.pkl'
        path = os.path.join('./data', file)
        if os.path.isfile(path):
            self.samples = pickle.load(open(path, "rb"))
            self.df = pd.read_pickle(os.path.join('./data', f'df_{name}.pkl'))
        else:
